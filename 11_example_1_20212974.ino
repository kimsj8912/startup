...
#define _DIST_ALPHA 0.7 // 가장 적당한 alpha 값으로 설정
...
float angle; // 측정 거리에 따른 서보 각도 변화에 이용
Servo myservo;
...
// get a distance reading from the USS
  ...
  dist_ema = dist_raw * alpha + (1 - alpha) * dist_ema; // EMA 보정 수식 
...
// 거리에 따른 서보 각도 제어
  if(dist_ema < 180) { // 거리 18cm 이하: 0
      myservo.writeMicroseconds(_DUTY_MIN);
   }
  else if(180 <= dist_ema <= 360){ // 거리에 비례하여 0~180 사이에서 연속적으로 변화
       angle = 10.28 * dist_ema - 1300;
...
// dist_min 과 dist_max 사이에 측정값이 있으면 LED를 켬
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading == 0.0) reading = dist_prev; // 범위를 벗어나면 이전 측정값 반환
