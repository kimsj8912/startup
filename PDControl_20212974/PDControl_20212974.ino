...
// Distance sensor
#define _DIST_ALPHA 0.6   // [2959] ema 필터에 적용할 알파값
...
float a, b, c, d, e, f, g; // 적외선 센서 값 보정을 위한 상수

MedianFilter<> filter; // IR 필터링
...
////////////////////
  ...
  if(filter.ready()) { // IR 필터링
      float dist = filter.read();
  // get a distance reading from the distance sensor
      dist_raw = dist;   // [2959] ??에 필터링된 측정값 저장
  ...
  // duty_target = f(duty_neutral, control)
      if(error_curr < 0) {
        duty_target = _DUTY_NEU - 0.5 * control;
      }
      else {
